...
At widths from `100` through `119` columns, the normal screen remains fully interactive in a narrow layout. Each physical sequencer line contains one 16-step bank with the same fixed-width event cells; additional banks use continuation lines within the track block, and adjacent logical track blocks retain their non-interactive horizontal dividers. Up/Down navigation follows these 16-step physical rows, while bank navigation remains 16 steps. Header, global-row, status, legends, and card labels use compact text while retaining recording, transport, pattern, tempo, selection, and audio-status indicators. The detail panel and overlays shrink or wrap to the available width. At `100x34` or larger, all project and audio behavior remains active.
...
The interface uses an explicit rendering theme selected by `--theme dark`, `--theme light`, or `--theme high-contrast`; `dark` is the default. Theme selection is a UI preference and is not persisted in project files. Each logical track has a fixed identity color in every theme profile; the color is used for its label, continuation marker, divider, and ordinary step glyphs. Occupied step cells retain a fixed-width background rectangle with an explicit foreground/background pair. Selected, playing, and selected-plus-playing cells use distinct explicit pairs and take precedence over track identity colors. Condition/retrigger emphasis remains text- and modifier-visible. Important state remains text- or modifier-visible so color is supplementary rather than the only signal. Theme colors are centralized and profile-specific; ordinary terminal content is not allowed to depend on a terminal's default foreground when it is rendered over a tinted cell.
