...
The kick is generated by a sine oscillator with exponential pitch and amplitude envelopes plus a very short synthesized noise/click transient.

- `tone`: maps the settled fundamental exponentially from approximately 35 Hz to 90 Hz. The pitch envelope begins around four times the settled pitch and falls over approximately 30 ms.
- `decay`: maps exponentially from approximately 50 ms to 1.2 seconds.

Defaults: tone 50%, decay 35%.

### 3.3 Snare drum

The snare combines two low body oscillators with high-pass-filtered pseudorandom noise. No stored noise sample is permitted.

- `tone`: jointly raises the body frequencies and noise high-pass cutoff while crossfading from a body-heavy sound at 0% toward a brighter, noise-heavy sound at 100%.
- `decay`: maps exponentially from approximately 40 ms to 1 second.

Defaults: tone 50%, decay 35%.

### 3.4 Hi-hat

The hi-hat combines at least six inharmonically related square oscillators, with fixed frequency ratios, through high-pass/band-pass filtering. Oscillators must use an alias-reduction technique suitable for the selected sample rate.

- `tone`: raises the metallic oscillator/filter region from a dark setting around 3 kHz toward a bright setting around 12 kHz, clamped below Nyquist.
- `decay`: maps exponentially from approximately 10 ms to 500 ms.
