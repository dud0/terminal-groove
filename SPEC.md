...
- `C` opens a compact horizontal Voicing editor on Chord or FM over the selected track's parameter section, keeping the sequencer visible. Left/Right selects Shape, Arp, Type, or Rate; Up/Down changes the selected value and stops at list boundaries; PageUp/PageDown moves between steps. Shape order begins with `1 (Mono)`, `1-3`, and `1-5`, followed by the existing three- and four-note shapes. Type and Rate remain remembered but are disabled while Arp is off. Note triggers show their values, ties show inherited source values read-only, and empty steps edit input defaults. Voicing settings are not BASE/LOCK parameters. Track labels show `M` or `C` for the current input default.
...
- Insert effects gate distortion, phaser, and flanger independently. Their static mappings, modulation-rate conversions, feedback coefficients, and delay scaling are cached while parameters are settled; phaser all-pass coefficients update in eight-frame control blocks with per-sample interpolation while smoothers and phase continue at audio rate. Effect tails use a bounded, allocation-free drain when input becomes silent. Ordinary live and preview chains exist only for tracks without voicing pools; the two live and two preview groups on each of Chord and FM retain independent chains. Flanger channels are preallocated for the documented 15 ms maximum delay at the selected sample rate.
