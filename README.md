...
FM is fixed track 10 (`Shift+0` or `)`). Its parameter bank provides carrier waveform (`w`), modulator ratio (`q`), Amount (`m`), Feedback (`f`), Brightness (`c`), Pitch LFO (`i`), and ADSR (`a/d/s/r`). The default electric-keys patch is Sine, ratio 2, Amount 35%, Feedback 8%, Brightness 72%, ADSR 0/55/55/40%, octave 3, level 80%, center pan, and 20% reverb send.
