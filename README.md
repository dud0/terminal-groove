...
The current strict schema uses `format_version: 8`; versions 1–7 and unknown versions are rejected without migration. Projects contain 1–100 dynamic patterns. Open the pattern dialog with `Ctrl+P` to navigate and insert, duplicate, copy, cut, paste, or delete patterns. Drum and note events carry a Boolean accent, while Bass notes can also arm a fixed-time slide into the next note. Eligible continuous parameters—including Chord/Lead oscillator mix, pulse width, and sub level—can each have an independent track-level LFO, opened from the parameter editor with `Shift+L`.
