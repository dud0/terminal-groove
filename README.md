...
A real-time terminal groovebox for Linux and macOS. It provides ten independently looping 1–64-step sequences, procedural drum voices including Tom, Cymbal, and Rimshot, a monophonic resonant Bass synth, a polyphonic Chord synth with two alternating four-voice groups, a monophonic Lead synth, and a four-operator FM synth with shared single-note/chord voicings. Projects are strict versioned JSON with undo/redo, parameter locks, and per-parameter LFOs. Chord and FM sequencer degrees produce diatonic shapes with inversions and optional arpeggiation.
...
Run with `cargo run --release`, optionally passing a project path, `--audio-device <exact-name>`, or `--audio-buffer <frames>`. Use `--list-audio-devices` to discover output names. The terminal must be at least 120 columns by 34 rows for the full fader layout.
...
The current strict schema uses `format_version: 25`. Versions 21 through 24 are migrated on load: canonical nine-track v21 projects gain the default FM track and an empty 16-step FM sequence in every pattern; v22 adds contextual Chord/FM shape defaults; v21 through v23 discard removed Chord spread values and locks; and v21 through v24 discard legacy Chord chorus values and locks. Other versions and malformed projects are rejected, and saves always emit v25. Top-level tracks contain shared configuration only; every sequence belongs to its pattern. Projects contain 1–100 dynamic patterns. FM supports ordinary notes, ties, shared single-note/chord voicings, and arpeggiation, but never slide. Eligible continuous parameters—including each FM operator's Level and Feedback, Brightness, ADSR, and Pitch—can each have an independent track-level LFO with optional trigger reset and a configurable starting phase, opened from the parameter editor with `Shift+L`. The global `Ducking` card (`d`) provides an optional kick-keyed sidechain compressor for Bass, Chord, Lead, and FM; its depth, attack, and release are edited together and default to Off.
